...
byte-for-byte untouched.** This is the fix for
[issue #219](https://github.com/fideus-labs/ngff-zarr/issues/219), where an
earlier "read, erase, re-write" upgrade destroyed the array data it was meant to
preserve. In-place upgrades that cross the Zarr v2/v3 boundary in the *upgrade*
direction (0.4 to 0.5 or 0.4 to 0.6) are also metadata-only: each array's Zarr v3
`zarr.json` is given a `v2` chunk-key encoding so the existing chunk binaries
resolve unchanged. The reverse -- an in-place *downgrade* across that boundary
(0.5/0.6 to 0.4) -- cannot preserve chunk keys and raises a `ValueError`; pass an
`output` store instead.
...
nz.to_ngff_zarr('image.ome.zarr', multiscales, version='0.5')
...
image = nz.to_ngff_image(np.zeros((4, 32, 32), dtype=np.uint8),
                         dims=['z', 'y', 'x'])
multiscales = nz.to_multiscales(image, scale_factors=[2])
nz.to_ngff_zarr('image_v04.ome.zarr', multiscales, version='0.4')
